...
def _touch_last_arm(name_s: str) -> None:
    """0.7: stamp a persistent 'armed recently' marker (mtime) for NAME. The PreToolUse re-arm hook
    reads this mtime + a grace window to tell a TRANSIENT deaf gap (normal, between a one-shot
    delivery and the re-arm) from a PATHOLOGICAL one (re-arm skipped for a while). It persists
    across the listen's exit (unlike the lease, which release_listener deletes)."""
    try:
        d = boards.boards_root() / ".last_arm"
        d.mkdir(parents=True, exist_ok=True)
        (d / name_s).touch()
    except OSError:
        pass
