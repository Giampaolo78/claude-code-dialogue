...
# dest=coordinator, doesn't wake the owners. (protocol fix 2026-06-13)
SYSTEM_BOARD = "system"
...
def _watchdog_lease_path() -> Path:
    return boards.boards_root() / ".watchdog.json"


def watchdog_pid():
    """The pid of the live watchdog for THIS project, or None. Single-process marker (the daemon runs
    forever, so the listener lease -- with expires_at -- doesn't fit; this is a plain pid file). Prunes
    a stale lease, with the pid>0 guard (os.kill(0,...) has special POSIX semantics, would false-alive)."""
    import os
    p = _watchdog_lease_path()
    if not p.is_file():
        return None
    try:
...
def stop() -> int:
    ...
    if pid:
        try:
            ...
            pass
    try:
        _watchdog_lease_path().unlink(missing_ok=True)
    except OSError:
        pass
    return pid or 0
...
def _notify_macos(text: str) -> bool:
    # escape backslash + double-quote so a quote in the alert text can't break the AppleScript string
    safe = text.replace("\\", "\\\\").replace('"', '\\"')
    try:
        subprocess.run(
            ["osascript", "-e",
             f'display notification "{safe}" with title "dialogue watchdog"'],
...
def _emit(wake: dict, conv: str, _board: str, state: dict) -> None:
    ...
    # All alerts go to the SYSTEM board, addressed to the COORDINATOR.
    # Why dest=coordinator and not dest=all: the owners' listen is a global
    ...
    if kind == "reminder":
        boards.say(conv, SYSTEM_BOARD, AUTHOR_WATCHDOG, text, dest="coordinator")
...
def run(interval_s: float = 60.0, threshold_s: float = 600.0,
    ...
    daemon = not one_pass
    if daemon:
        existing = watchdog_pid()  # prunes a stale lease; returns a LIVE pid or None
        if existing:
            print(f"[WATCHDOG] already running (pid {existing}) for this project "
                  f"-> not starting a second one.", flush=True)
            return
        # ATOMIC single-process claim: O_EXCL lets exactly ONE of two simultaneous arms win the lease.
        # A check-then-write (watchdog_pid then _atomic_write) is a TOCTOU: both arms see no lease and
        # both write -> two daemons, one orphan. The exclusive create closes that window.
        lease = _watchdog_lease_path()
        lease.parent.mkdir(parents=True, exist_ok=True)
        try:
            fd = os.open(str(lease), os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o644)
        except FileExistsError:
            other = watchdog_pid()
            print(f"[WATCHDOG] already running (pid {other}) for this project "
                  f"-> not starting a second one.", flush=True)
            return
        try:
