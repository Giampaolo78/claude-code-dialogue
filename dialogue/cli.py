...
def cmd_ping(args) -> int:
    from .guardiano import PING_TEXT
    text = PING_TEXT + (f" Coordinator note: {args.note}" if args.note else "")
    boards.say(args.conv, args.board, "coordinatore", text, dest=args.name)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pg.add_argument("--board", dest="board", default="coordinamento")
    pg.add_argument("--note", dest="note", default=None)
    pg.set_defaults(func=cmd_ping)

    w = sub.add_parser("watchdog", help="anti-freeze guardian (run by the coordinator)")
