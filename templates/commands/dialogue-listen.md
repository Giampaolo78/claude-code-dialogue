...
- Otherwise infer from this window's domain: frontend -> `claude-frontend`; backend -> `claude-backend`; data -> `claude-dati`; coordination / gate -> `coordinatore`.
...
1. Launch in BACKGROUND (run_in_background = true) exactly this:
   ...
   The listen is a GLOBAL cursor: it wakes you for EVERY message addressed to {all, <NAME>} on any board (not just `coordinamento`).
