...
4. **Leave a pointer on `coordinamento`** so they know the thread exists (the dedicated board doesn't wake them more, but they need to know to go there):
   ```
   dlg say <project> coordinamento <NAME> "Opened board <board> for the critical issue: <short> -> comparison there." --to <owner|all>
   ```
