...
2. **If CREATED** (you are the round's initiator): leave a kickoff on `coordinamento` so the others know to post. If JOINED, skip this step.
   ```
   dlg say <project> coordinamento <NAME> "RECAP-ROUND open on <board>: post your status there (working-on / plan / blocked-by / pending)." --to all
   ```
