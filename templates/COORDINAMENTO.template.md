...
- **Model B - Claude-coordinator (alternative):** a dedicated Claude instance
  (`coordinatore`) acts as integrator/gate/serializer between human and owners, with a
...
- Implicit DEFAULT from the roster (`dlg status`): if there is a `coordinatore` registered and
  ONLINE -> **Model B** applies. Otherwise -> **Model A** applies.
- Explicit override: the human can declare the model at the top of the `coordinamento` board
...
- **How an owner escalates to the human** (the human is not a listening instance): (1) in their
  ...
  `--to boss` on the `coordinamento` board as a durable record, which the human reviews via
...
2. **Integration / overview** -> the human via `dlg dashboard` (their cockpit over all
   boards), or by reading `coordinamento`. Owners report to them directly.
...
Activated by registering and bringing ONLINE a `coordinatore` instance.
...
`inbox`, `status`, `queue`, `ping`, `dashboard`, `watchdog`. `dlg <command> --help` for details.
...
- `coordinamento`: team coordination (introductions, REQ, ACK, handoff, announcements,
  escalation to the human via `--to boss`). It's the board on which the `listen` is armed.
- `sistema`: watchdog alerts. Addressed `--to boss` (Model A) or `--to coordinatore`
...
freeze) and instance deafness, and alerts the `sistema` board + macOS notification, indicating
